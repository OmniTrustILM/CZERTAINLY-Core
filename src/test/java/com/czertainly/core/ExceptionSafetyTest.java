 ...
 *
 * Rule 1: All concrete Throwable subclasses in com.czertainly.core must implement
 *         PlatformException so safeMessage() can gate message exposure at wire boundaries.
 *
 * Rule 2: No @RestControllerAdvice class may call Throwable.getMessage() or getCause()
 *         — use PlatformException.safeMessage() instead, log the cause via the Throwable itself.
 *
 * Rule 3 (frozen): No production class in com.czertainly.core may call Throwable.getMessage()
 *         anywhere. Existing violations are frozen in archunit_store/; only net-new calls fail
 *         the build. Fix frozen violations incrementally; when the store is empty this can
 *         become a plain @ArchTest.
 */
...
class ExceptionSafetyTest {
    ...
    private static final DescribedPredicate<JavaMethodCall> GET_MESSAGE_ON_THROWABLE =
            new DescribedPredicate<>("calls getMessage() on a Throwable or subtype") {
                ...
                public boolean test(JavaMethodCall call) {
                    if (!"getMessage".equals(call.getTarget().getName())) return false;
                    if (!call.getTarget().getRawParameterTypes().isEmpty()) return false;
                    try {
     ...
     * Existing violations are stored in archunit_store/ and must shrink over time.
     * When the store file is empty (all violations fixed) replace @Test + FreezingArchRule
     * with a plain @ArchTest on the rule.
     */
    @Test
